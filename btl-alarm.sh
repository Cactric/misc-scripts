...
timeNow=$(date +%s)
btlFinished=$(stat -c %Y "${XDG_STATE_HOME:-$HOME/.local/state}/"last-btl-*-finished | sort -n | head -n 1)
...
if [ $((timeNow - btlFinished)) -gt $permissibleSec ]; then
    ...
    notify-send -t 10000 "It has been more than a week since the oldest back up" -a "Backup alarm" -i backup
fi
